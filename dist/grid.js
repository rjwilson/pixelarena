// src/grid.ts
import { GRID_ROWS, GRID_COLS, TILE_SIZE } from './config';
/**
 ...
 */
function drawBaseTiles(ctx, grid) {
    for (let y = 0; y < GRID_ROWS; y++) {
        for (let x = 0; x < GRID_COLS; x++) {
 ...
 */
export function renderGrid(ctx, gameState) {
 ...
 */
export function pixelToGridCoords(pixelX, pixelY) {
 ...
 */
export function isWithinGrid(p) {
 ...
 */
export function occupyTile(grid, position, characterId) {
 ...
 */
export function freeTile(grid, position) {
    const tile = getTile(grid, position.x, position.y);
    if (tile) {
 ...
 */
export function findCharactersInRange(center, range, characters, includeCenter = false) {
    const charactersInRange = [];
 ...
 */
export function highlightTiles(ctx, tilesToHighlight, color) {
    ctx.fillStyle = color;
 ...
 */
export function getReachableTiles(startPos, range, grid, characters) {
    const reachable = [];
    const queue = [{ point: startPos, dist: 0 }];
 ...
 */
export function getTargetableTiles(startPos, range, grid) {
    const targetable = [];
    for (let y = 0; y < GRID_ROWS; y++) {
        for (let x = 0; x < GRID_COLS; x++) {
