import { rollDice, TILE_SIZE, D20_CRITICAL_HIT_THRESHOLD, D20_CRITICAL_MISS_THRESHOLD, DAMAGE_TEXT_COLOR, DAMAGE_TEXT_CRIT_COLOR, MISS_TEXT_COLOR, VFX_DURATION } from './config';
 ...
 */
export function processAttack(attacker, target, action, addVisualEffect) {
    ...
    if (result.hit) {
        ...
        if (result.crit) {
            ...
            if (action.damage) { // Check if damage function exists
                ...
                if (action.name === "Slash")
                    criticalDamageBonus = rollDice(6); // Warrior's Slash uses d6
                else if (action.name === "Firebolt")
                    criticalDamageBonus = rollDice(6, 2); // Mage's Firebolt uses 2d6
                else if (action.name === "Power Attack")
                    criticalDamageBonus = rollDice(8); // Warrior's PA uses d8
                else if (action.name === "Scimitar")
                    criticalDamageBonus = rollDice(6); // Goblin Grunt
                else if (action.name === "Shortbow")
                    criticalDamageBonus = rollDice(6); // Goblin Archer
                else
                    criticalDamageBonus = damageAmount; // Fallback: double the initial damage if specific dice aren't clear
            }
            else {
        ...
        addVisualEffect({
            ...
            duration: result.crit ? VFX_DURATION * 1.5 : VFX_DURATION,
            ...
            animation: result.crit ? (ctx, effect, now) => {
                ...
                ctx.fillText(effect.text || '', (effect.position.x + 0.5) * TILE_SIZE, (effect.position.y + 0.5) * TILE_SIZE + yOffset);
                ctx.restore();
            } : undefined
        });
        if (!target.isAlive) {
