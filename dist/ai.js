...
import { getReachableTiles, findCharactersInRange } from './grid';
export function getEnemyAIAction(aiCharacter, gameState) {
    ...
    if (aiCharacter.canMove) {
        ...
        let bestMoveForAction = null;
        for (const movePos of reachableTiles) {
            if (!aiCharacter.canAct)
                break; // No point evaluating actions if AI cannot act after moving
        ...
        let bestMoveToGetCloser = null;
        // Find the GEOGRAPHICALLY closest player
        let geographicallyClosestPlayer = null;
