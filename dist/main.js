// src/main.ts
import { GRID_ROWS, GRID_COLS, TILE_SIZE, PLAYER_WARRIOR_CONFIG, PLAYER_MAGE_CONFIG, ENEMY_MELEE_CONFIG, ENEMY_RANGED_CONFIG, INITIAL_PARTY_SETUP, INITIAL_ENEMY_SETUP, MOVABLE_TILE_COLOR, ATTACKABLE_TILE_COLOR, SELECTED_TILE_COLOR, HOVER_TILE_COLOR } from './config';
...
import { getEnemyAIAction } from './ai';
// --- Global Game State ---
...
let reachableMovementTiles = [];
let attackableTargetTiles = []; // Used by player actions
let specialAbilityTargetTiles = [];
// --- Visual Effects ---
let visualEffects = [];
/**
...
export function initializeGame() {
    ...
    INITIAL_PARTY_SETUP.forEach(setup => {
        let config;
        if (setup.configName === "Warrior")
            config = PLAYER_WARRIOR_CONFIG;
        else if (setup.configName === "Mage")
            config = PLAYER_MAGE_CONFIG;
        else {
            console.error(`Unknown player config name: ${setup.configName}`);
            return;
        }
    ...
    INITIAL_ENEMY_SETUP.forEach(setup => {
        let config;
        if (setup.configName === "Goblin Grunt")
            config = ENEMY_MELEE_CONFIG;
        else if (setup.configName === "Goblin Archer")
            config = ENEMY_RANGED_CONFIG;
        else {
            console.error(`Unknown enemy config name: ${setup.configName}`);
            return;
        }
...
function startTurn() {
    if (!gameState || gameState.isCombatOver)
        return;
...
function processEnemyActionPhase(enemyCharacter) {
    if (!gameState || !enemyCharacter.isAlive || gameState.isCombatOver || enemyCharacter.type === CharacterType.PLAYER) {
        if (enemyCharacter.type !== CharacterType.PLAYER && (!enemyCharacter.isAlive || (gameState && gameState.isCombatOver))) {
            if (gameState && gameState.activeCharacterId === enemyCharacter.id)
                endTurn();
        }
        return;
    }
    if (!enemyCharacter.canMove && !enemyCharacter.canAct) {
        ...
        if (typeof setTimeout !== 'undefined')
            setTimeout(() => { if (gameState.activeCharacterId === enemyCharacter.id)
                endTurn(); }, 10);
        else {
            if (gameState.activeCharacterId === enemyCharacter.id)
                endTurn();
        }
        return;
    }
    ...
    const endAITurn = () => { if (gameState.activeCharacterId === enemyCharacter.id)
        endTurn(); };
...
function endTurn() {
    if (!gameState || (gameState.isCombatOver && gameState.activeCharacterId === null))
        return;
...
function gameLoop(timestamp) {
    ...
    if (ctx) {
        ...
        gameState.characters.forEach(char => {
            renderCharacter(ctx, char, char.id === gameState.selectedCharacterId && char.type === CharacterType.PLAYER, char.id === gameState.activeCharacterId);
        });
        const now = Date.now();
...
export function handleCanvasClick(event) {
    ...
    if (!activePlayerCharacter || !activePlayerCharacter.isAlive)
        return;
...
export function handleActionButtonClick(actionType) {
    if (!gameState || gameState.isCombatOver || !gameState.isPlayerTurn)
        return;
    const activeCharacter = gameState.characters.find(c => c.id === gameState.activeCharacterId);
    if (!activeCharacter || !activeCharacter.isAlive)
...
function performAttack(attacker, target, action) {
    if (!attacker.canAct) {
...
function performSpecialAbility(caster, target, action) {
    if (!caster.canAct) {
...
function updateActionButtonsForCurrentCharacter() {
    if (!gameState)
        return;
    const activeCharacter = gameState.characters.find(c => c.id === gameState.activeCharacterId);
...
// --- Test Hooks ---
export function getGameState_TEST_HOOK() {
    return gameState;
}
export function clearVisualEffects_TEST_HOOK() {
    visualEffects = [];
}
export function getAttackableTargetTiles_TEST_HOOK() {
