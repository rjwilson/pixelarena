// src/ui.ts
import { Character, GameState, GameAction, ActionType, CharacterType } from './types';
import { MAX_ACTION_LOG_MESSAGES, UI_FONT } from './config';

// --- DOM Element References ---
let playerCharactersDisplay: HTMLElement;
let enemyCharactersDisplay: HTMLElement;
let actionLogDisplay: HTMLElement;
let currentTurnCharacterSpan: HTMLElement;
let actionsContainer: HTMLElement;
let gameMessageBox: HTMLElement;
let messageTitle: HTMLElement;
let messageText: HTMLElement;
let messageCloseButton: HTMLButtonElement;

// Action Buttons
let moveButton: HTMLButtonElement;
let attackButton: HTMLButtonElement;
let specialButton: HTMLButtonElement;
let endTurnButton: HTMLButtonElement;

type ActionButtonCallback = (action: ActionType | 'SPECIAL' | 'END_TURN') => void;
type ResetGameCallback = () => void;
...
export function initializeUI(
    ...
    onEndTurnClick: () => void,
    onResetGameClick: ResetGameCallback
): void {
    ...
    messageCloseButton = document.getElementById('message-close-button') as HTMLButtonElement;

    // Get action buttons
    moveButton = document.getElementById('move-button') as HTMLButtonElement;
    attackButton = document.getElementById('attack-button') as HTMLButtonElement;
    specialButton = document.getElementById('special-button') as HTMLButtonElement;
    endTurnButton = document.getElementById('end-turn-button') as HTMLButtonElement;
 ...
 */
export function updateCharacterDisplays(characters: Character[]): void {
    if (!playerCharactersDisplay || !enemyCharactersDisplay) return;
    ...
    characters.forEach(char => {
        ...
        if (hpPercentage <= 25) healthBar.style.backgroundColor = '#f87171'; // red-400
        else if (hpPercentage <= 60) healthBar.style.backgroundColor = '#facc15'; // yellow-400
        else healthBar.style.backgroundColor = '#4ade80'; // green-400
 ...
 */
export function addMessageToActionLog(message: string, gameState?: GameState): void {
    if (!actionLogDisplay) return;
 ...
 */
export function updateActionLog(messages: string[]): void {
    if (!actionLogDisplay) return;
 ...
 */
export function updateTurnIndicator(activeCharacter: Character | null): void {
    if (!currentTurnCharacterSpan) return;
 ...
 */
export function showGameMessage(title: string, text: string, winner?: CharacterType | null): void {
    if (!gameMessageBox || !messageTitle || !messageText || !messageCloseButton) return;
 ...
 */
export function hideGameMessage(): void {
    if (!gameMessageBox) return;
